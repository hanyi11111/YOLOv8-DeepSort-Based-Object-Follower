...
class YOLO:
    ...
    def predict(self, source, **kwargs):
        ...
        # PATCH: 复用 Predictor，避免每帧新建导致反复 Fusing layers / 打印 summary
        if getattr(self, "_yolo_predictor", None) is None:
            self._yolo_predictor = self.PredictorClass(overrides=overrides)
            self._yolo_predictor.args.imgsz = check_imgsz(self._yolo_predictor.args.imgsz, min_dim=2)  # check image size
        self._yolo_predictor.setup(model=self.model, source=source)
        return self._yolo_predictor()
