...
def torch_distributed_zero_first(local_rank: int):
    ...
    )  # compatibility for torch builds without full distributed support
    if initialized and hasattr(dist, "barrier") and local_rank not in {-1, 0}:
        dist.barrier(device_ids=[local_rank])
    yield
    if initialized and hasattr(dist, "barrier") and local_rank == 0:
