...
def _nms_fallback(boxes, scores, iou_thres):
    # Pure PyTorch NMS fallback for environments without torchvision C++ ops.
    if boxes.numel() == 0:
    ...
    return torch.stack(keep) if keep else torch.empty((0,), device=boxes.device, dtype=torch.long)


def _safe_nms(boxes, scores, iou_thres):
    try:
        return torchvision.ops.nms(boxes, scores, iou_thres)
    except Exception as e:
        if not getattr(_safe_nms, "_warned", False):
            LOGGER.warning(f"torchvision.ops.nms unavailable, using PyTorch fallback NMS: {e}")
            _safe_nms._warned = True
        return _nms_fallback(boxes, scores, iou_thres)
...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        i = _safe_nms(boxes, scores, iou_thres)  # NMS
